...
"@
    ...
    # Change handlers
    $propertyCombo.Add_SelectionChanged({ if ($changeHandler) { Invoke-Command -ScriptBlock $changeHandler } })
    $operatorCombo.Add_SelectionChanged({ if ($changeHandler) { Invoke-Command -ScriptBlock $changeHandler } })
    $valueBox.Add_TextChanged({ if ($changeHandler) { Invoke-Command -ScriptBlock $changeHandler } })

    # Remove button handler
    $removeBtn.Add_Click({
        $Panel.Children.Remove($filterControl)
        $script:Filters.Remove($filterData)
        if ($changeHandler) { Invoke-Command -ScriptBlock $changeHandler }
    })
